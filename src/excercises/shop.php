...
final class Shop extends AbstractNormForm
{
     ...
     *
     * @var string START Key für $_GET- und $_SESSION-Eintrag den Starteintrag für den angezeigten Ausschnitt der Produktliste festlegt @see src/basetemplates/pagination.tpl
     ...
     * @var string $oldsearch Hilfsvariable, um den alten Wert von $search zwischenzuspeichern, um ihn später mit dem neuen vergleichen zu können.
     ...
     * @var string $pagecount Anzahl der Seiten, auf denen die Produktliste dargestellt werden kann, abhängig von der Konstante DISPLAY
     * @var string $startprevious Startwert für die LIMIT-Klausel für das Select für den Previous-Link
     * @var array $pagenumber Array mit den Startwerten aller Seiten, die über einen Link der mit der Seitennummer benannt ist, direkt angesprungen werden können
     * @var string $current_page Hilfsvariable, die die Seitennummer der aktuell angezeigten Seite der Produktliste beinhaltet
    ...
    public function __construct(View $defaultView, $templateDir = "templates", $compileDir = "templates_c")
    {
        ...
        $this->currentView->setParameter(new GenericParameter("sortKey", self::SORT));
    ...
    protected function business()
    {
        if (isset($_POST[self::PID])) {
            ...
            $this->currentView->setParameter(new GenericParameter("pageArray", $this->fillpageArray()));
            $this->currentView->setParameter(new PostParameter(Shop::SEARCH, true));
        } else {
    ...
    private function fillpageArray()
    {
        ...
        //##
        return $pageArray = array( 0 => array('idproduct' => 1, 'product_name' => 'Passivhaus', 'price' => 300000,00), 1 => array('idproduct' => 2, 'product_name' => 'Niedrigenergiehaus', 'price' => 200000,00), 2 => array('idproduct' => 3, 'product_name' => 'Almgrundstück', 'price' => 100000,00));
        //*/
        /*--
     ...
     *
     * Der Suchbegriff $this->search, der vom Benutzer im GET-Formular eingegeben wurde, für die LIKE-Klausel im Select @see Shop::setPageCount()
     * wird ermittelt, validiert und im Session-Array zwischengespeichert, damit er beim Blättern über Seitenaufrufe hinweg erhalten bleibt.
     *
     * Der Wert der vorherigen Suche wird in $this->oldsearch zwischengespeichert. Der Startwert $this->start wird immer auf 0 gesetzt, wenn eine neue Suche gestartet wurde.
     * Erkannt wird das daran, dass in $this->oldsearch ein anderer Wert gespeichert ist als in $_GET[self::SEARCH] --> @see Shop::setStart()
     *
     * Wurde der Suchbegriff mit $_GET mitgegeben wird er von dort übernommen und im Array $_SESSION hinterlegt.
     * Kommt er aus $_GET, wird mit Utilies::isSearchString() sichergestellt, dass es sich um einen einzelnen Suchbegriff handelt.
     ...
     *
     * Ermittelt über @see Shop::getPageCount(), die Anzahl der Seiten, die zur Darstellung aller Datensätze notwendig sind.
     * Ermittelt den Startwert für alle Links der Blätterfunktion und die LIMIT-Klausel im Select auf die Tabelle onlineshop.product @see Shop::setStart(),
     * Außerdem werden die Seitenzahlen für alle Seiten ermittelt, die durchblättert werden können und als Links mit dem Startwert im Template implementiert sind.
     * $this->pagenumber enthält für jede Seite $i deren Startwert. Die Seitenzahl $i ist der Key im Array.
     * Für die aktuelle Seite wird die Seitenzahl als Text in HTML ausgegeben und nicht mit einem mit der Seitenzahl benannten Link hinterlegt. @see templates/pagination.tpl
     */
     ...
     * Hilfsmethode für das Blättern in der Produktliste
     *
     * Ermittelt die Anzahl der Seiten $this->pagecount['count'], die notwendig ist um alle Produkte aufzulisten, wenn nur soviele Produkte pro Seite angezeigt werden,
     * wie in der Konstante DISPLAY festgelegt ist. templates/pagination.tpl
     *
     * Dazu muss im SELECT eine Gruppenfunktion zum Zählen genutzt werden und ein Alias, der count heißt (AS count). Dann wird dieser Alias count im associativen Rückgabe-Array zum Key.
     * $this->rowcount['count'] wird in @see isValidStart() benötigt. Durch die Verwendung einer Objekt-Variable $this->... ist sie dort automatisch sichtbar.
     *
     * Es werden nur aktive Produkte berücksichtigt, bei denen die Spalte onlineshop.product.active='1' ist.
     * Es werden nur Produkte gezählt, die dem Suchkriterium entsprechen. Das Suchkriterium aus dem Suchfeld (GET-Formular) kann leer sein.
     * Dann gibt es keine Einschränkung.
     * Suchfelder über die LIKE-Klausel sind die Spalten onlineshop.product.product_name, -.short_description, -.long_description.
     * Der Suchbegriff wird in @see Shop::setSearch() ermittelt.
     *
     * @throws DatabaseException Diese wird von allen $this->dbAccess Methoden geworfen und hier nicht behandelt.
     *         Die Exception wird daher nochmals weitergereicht (throw) und erst am Ende des Scripts behandelt.
    ...
    {
        //##
        $this->rowcount['count'] = 3; // Es sind 3 statische Datensätze
        $this->pagecount = 2; // Damit man das Blättern sieht auf 2 gesetzt. Auf beiden Seiten wird das gleiche angezeigt. Erst durch DB-Zugriff wird auf jeder Seite das richtige angezeigt.
        //*/
        /*--
     ...
     * Hilfsmethode für das Blättern in der Produktliste
     *
     * Der Startwert $this->start für die LIMIT-Klausel im Select @see Shop::fillpageArray() für die aktuelle Seite wird ermittelt, validiert und im Session-Array gespeichert.
     * Wurde $_GET[self::START] mitgegeben, wird der Wert von dort übernommen und es wird mit @see Utilies::isValidStart() sichergestellt,
     * dass es sich um eine positive Integerzahl handelt und der Wert die Anzahl der Sätze in der Tabelle onlineshop.product nicht überschreitet.
     * Ist $_GET[self::START] nicht befüllt, aber $_SESSION[self::START] beinhaltet einen Wert, wird er aus der Session übernommen.
     * In allen anderen Fällen wird er auf 0 gesetzt.
     *
     * Wurde ein neuer Suchbegriff eingegeben, wird der Startwert auf 0 gesetzt, das heißt auf der ersten Seite der Produktliste gestartet.
     *
     * Tabelle onlinshop.product übersteigen, aber kein XSS. Die Startwerte werden in @see src/basetemplates/pagination.tpl ausgegeben.
     */
    private function setStart()
    {
        ...
        if (isset($_SESSION[self::SEARCH]) && isset($_POST[self::SEARCH]) && $this->oldsearch !== $_POST[self::SEARCH]) {
     ...
     *
     * Mit @see Utilities::isInt() wird geprüft ob $_GET[self::START] einen positiven Integerwert enthält oder 0. Dadurch wird XSS verhindert.
     * Zusätzlich wird geprüft, ob der Startwert kleiner ist als die Gesamtzahl der Sätze $this->rowcount['count'] in der Tabelle onlineshop.product, die in @see Shop::setPageCount() ermittelt wird.
     * Das verhindert Forced Browsing mit sinnlosen Werten. Wäre der Startwert größer als die Anzahl der Sätze in onlineshop.product würde eine Leere Seite angezeigt.
     *
     */
    ...
    /**
     * Hilfsmethode für die Sortierung der Produktliste befüllt die Variable $order_by mit der ORDER BY-Klausel für den Select in @see Shop::fillpageArray()
     *
     * Durch Shop:setSort() wird die Variable $this->sort befüllt.
     * Durch deren Inhalt wird die Spalte bestimmt, nach der die Datensätze der Tabelle onlineshop.product in @see Shop::fillpageArray() sortiert werden.
     * Es wird nur die Sortierreihenfolge ASC unterstützt.
     * Durch die Implementierung einer PullDownBox mit mehreren Einträgen, könnte man beliebige Sortierungen unterstützen.
     *
     ...
     *
     * Die Spalte $this->sort für die Sortierung in der ORDBY BY-Klausel im Select @see Shop::fillpageArray() für die aktuelle Seite wird ermittelt, validiert und im Session-Array gespeichert.
     * Wurde sie mit $_GET[self::SORT] mitgegeben wird sie von dort übernommen und im Array $_SESSION hinterlegt, um über Seitenaufrufe hinweg erhalten zu bleiben.
     * Kommt sie aus $_GET, wird mit @see Shop::isValidSort() sichergestellt, dass es sich um eine gültige Sortierung handelt.
     ...
     * array_map() wird hier zu Demonstrationszwecken eingesetzt. Ein direkter Aufruf von Utilities::isInt() in der foreach-Schleife würde auch genügen.
     * isValidPid() prüft mit Utilities::isInt() jeden KEY im Array $_POST['pid'][KEY] auf positives Integer oder 0 um XSS zu verhindern.
     * Das ist nur einer (siehe vorher). Zusätzlich wird für diesen KEY geprüft, ob dieser in der Tabelle onlineshop.product vorkommt, um Forced Browsing mit sinnlosen pids zu verhindern.
     *
     * @return bool false, wenn eine pid kein positives Integer ist, oder nicht in der Datenbank vorkommt. Ansonsten true.
     * @throws DatabaseException Diese wird von allen $this->dbAccess Methoden geworfen und hier nicht behandelt.
     *         Die Exception wird daher nochmals weitergereicht (throw) und erst am Ende des Scripts behandelt.
     */
